        ...
        for (var c in b)
            if (b.hasOwnProperty(c)) {
                ...
                if (d.holder_data) {
                    ...
                    f && (e.fluid && d.setAttribute("src", g({ctx: s,dimensions: f,template: e.theme,ratio: t,holder: e})), e.textmode && "exact" == e.textmode && (e.exact_dimensions = f, d.setAttribute("src", g({ctx: s,dimensions: e.dimensions,template: e.theme,ratio: t,holder: e}))))
                }
            }
    }
    function l(b, c) {
        var d = {theme: e(u.themes.gray, {})}, f = !1;
    ...
    a(function() {
        ...
        var buildNav = function(nav_ul) {
            ...
            function navItem(item) {
                ...
                var title = item.data('title') || item.text();
                var li = $('<li><a href="#' + id + '">' + title + '</a></li>');
                return li;
            }

            var h3_list = $('.bs-docs-container h3');
            // var h1_ids = [];
            // h1_list.each(function(){ h1_ids.push(this.id); });
            var h3_last = h3_list.length - 1;

            h3_list.each(function(i3, h3_item) {
                h3_item = $(h3_item);

                var h3_nav = navItem(h3_item);

                var h4_list;
                if (i3 != h3_last) {
                    h4_list = h3_item.nextUntil('h3', 'h4');
                    if (!h4_list.length) {
                        h4_list = h3_item.nextUntil('h3', 'h5');
                    }
                } else {
                    h4_list = h3_item.nextAll('h4');
                    if (!h4_list.length) {
                        h4_list = h3_item.nextAll('h5');
                    }
                }
                if (h4_list.length > 0) {
                    var h4_ul = $('<ul>').addClass('nav');
                    h4_list.each(function(i4, h4_item) {
                        h4_ul.append(navItem($(h4_item)));
                    });
                    h3_nav.append(h4_ul);
                }
                nav_ul.append(h3_nav);
                h3_nav.hide();
                h3_nav.fadeIn();
            });
        };
        ...
        d.scrollspy({target: ".bs-sidebar"}), c.on("load", function() {
            d.scrollspy("refresh")
